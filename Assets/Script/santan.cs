...
public class santan : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // 미사일 자체를 지울 필요는 없어요!
        if (elapsedTime > 0.3f && target != null && elapsedTime < homingDuration)
        {
            LookAtTarget();
        }
    ...
    void Reflect(Transform playerTransform)
    {
        ...
        // 2. 유도 다시 시작 (Sniper_Missile의 경우 homingDuration을 여기서 늘려주면 좋습니다)
        elapsedTime = 0f;
        homingDuration = 5f;
